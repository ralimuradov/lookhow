 ...
 #------------------------------------------------------------------------   
    ...
    async def receive_messages(self, websocket) -> None:
        try:
            ...
            self.query_one('#code-area').text = recv_message
        except websockets.ConnectionClosedOK:
            self.notify('Connection closed.', severity='warning', timeout=4)
    ...
    async def update_text_area(self) -> None:
        async with websockets.connect(IP_PORT) as websocket:
            ...
            _, pending = await asyncio.wait([receive_task], return_when=asyncio.FIRST_COMPLETED)
